...
var page = {
    ...
    common: function() {
        ...
        var getContent = $(".main_bottom, .sub_content");
        quickMenu.addClass("on");
        if(getContent.length > 0) {
            var getQuickTop = getContent.offset().top + 30;
            quickMenu.css("top", getQuickTop);
        }
        else {
            var getQuickTop = quickMenu.offset().top - 30;
        }
    ...
    main: function() {
        if($("#main_slide").length > 0) {
            $("#main_slide").removeClass("load").slick({
                autoplay: true,
                autoplaySpeed: 5000,
                arrows:true,
                dots:false
...
//명예의전당 갤러리
        if($("#honor_gallery_slide").length > 0) {
            $("#honor_gallery_slide img").imgpreload(function() {
                var gallerySlide = $("#honor_gallery_slide").lightSlider({
					...
					onBeforeSlide: function(el){
						moveSlider(el.getCurrentSlideCount());
					}
                });
            });
        }
        //명예의전달 년도 슬라이드
        if($("#honor_year_tab").length > 0) {
            $("#honor_year_tab .tab_menu").slick({
                infinite:false,
                dots:false,
                variableWidth:true,
                swipeToSlide:true
    ...
    museum: function() {
        ...
        if($("#museum_detail_slide").length > 0) {
            ...
            $("#museum_detail_thumb .btn_thumb").eq(0).addClass("active");
        ...
        if($("#museum_detail_slide").length > 0) {
            ...
            var btnMuseumThumb = $("#museum_detail_thumb .btn_thumb");
            if(btnMuseumThumb.length == 1) $("#museum_detail_thumb").hide();
            else if(btnMuseumThumb.length == 3) $("#museum_detail_thumb").addClass("mid");
            else if(btnMuseumThumb.length < 3) $("#museum_detail_thumb").addClass("half");
            btnMuseumThumb.eq(0).addClass("active");
